from enum import Enum, StrEnum


class Environment(StrEnum):
    DEVELOPMENT = "development"
    TESTING = "testing"
    PRODUCTION = "production"


class HealthStatus(StrEnum):
    HEALTHY = "healthy"
    READY = "ready"
    NOT_READY = "not_ready"


class CustomerStatus(StrEnum):
    NEW = "new"
    PENDING_VERIFICATION = "pending_verification"
    VERIFIED = "verified"
    SUSPENDED = "suspended"
    REJECTED = "rejected"


class AuditEventType(str, Enum):
    LOGIN_SUCCESS = "LOGIN_SUCCESS"
    LOGIN_FAILURE = "LOGIN_FAILURE"
    LOGOUT = "LOGOUT"
    PASSWORD_CHANGE = "PASSWORD_CHANGE"

    FILE_UPLOAD = "FILE_UPLOAD"
    FILE_DOWNLOAD = "FILE_DOWNLOAD"
    FILE_DELETE = "FILE_DELETE"
    CUSTOMER_DOCUMENT_CREATED = "CUSTOMER_DOCUMENT_CREATED"

    USER_CREATED = "USER_CREATED"
    USER_UPDATED = "USER_UPDATED"
    USER_DELETED = "USER_DELETED"
    USER_STATUS_CHANGED = "USER_STATUS_CHANGED"

    CUSTOMER_CREATED = "CUSTOMER_CREATED"
    CUSTOMER_UPDATED = "CUSTOMER_UPDATED"
    CUSTOMER_STATUS_CHANGED = "CUSTOMER_STATUS_CHANGED"

    ACCESS_DENIED = "ACCESS_DENIED"

    VERIFICATION_CASE_CREATED = "VERIFICATION_CASE_CREATED"
    VERIFICATION_CASE_STATUS_CHANGED = "VERIFICATION_CASE_STATUS_CHANGED"

    VERIFICATION_DOCUMENT_TYPE_CREATED = "VERIFICATION_DOCUMENT_TYPE_CREATED"
    VERIFICATION_DOCUMENT_TYPE_UPDATED = "VERIFICATION_DOCUMENT_TYPE_UPDATED"
    VERIFICATION_DOCUMENT_TYPE_STATUS_CHANGED = (
        "VERIFICATION_DOCUMENT_TYPE_STATUS_CHANGED"
    )

    VERIFICATION_PROVIDER_REQUEST = "VERIFICATION_PROVIDER_REQUEST"

    OCR_REQUESTED = "OCR_REQUESTED"
    OCR_COMPLETED = "OCR_COMPLETED"
    OCR_FAILED = "OCR_FAILED"

    DOCUMENT_EXTRACTION_REQUESTED = "DOCUMENT_EXTRACTION_REQUESTED"
    DOCUMENT_EXTRACTION_COMPLETED = "DOCUMENT_EXTRACTION_COMPLETED"
    DOCUMENT_EXTRACTION_FAILED = "DOCUMENT_EXTRACTION_FAILED"

    DOCUMENT_EXTRACTION_REVIEW_UPDATED = "DOCUMENT_EXTRACTION_REVIEW_UPDATED"
    DOCUMENT_EXTRACTION_REVIEW_APPROVED = "DOCUMENT_EXTRACTION_REVIEW_APPROVED"
    DOCUMENT_EXTRACTION_REVIEW_REJECTED = "DOCUMENT_EXTRACTION_REVIEW_REJECTED"

    AI_PROMPT_CREATED = "AI_PROMPT_CREATED"
    AI_PROMPT_VERSION_CREATED = "AI_PROMPT_VERSION_CREATED"
    AI_PROMPT_ACTIVATED = "AI_PROMPT_ACTIVATED"
    AI_PROMPT_DEACTIVATED = "AI_PROMPT_DEACTIVATED"
    AI_PROMPT_ASSIGNED = "AI_PROMPT_ASSIGNED"

    KNOWLEDGE_DOCUMENT_CREATED = "KNOWLEDGE_DOCUMENT_CREATED"
    KNOWLEDGE_DOCUMENT_VERSION_CREATED = "KNOWLEDGE_DOCUMENT_VERSION_CREATED"
    KNOWLEDGE_DOCUMENT_ACTIVATED = "KNOWLEDGE_DOCUMENT_ACTIVATED"
    KNOWLEDGE_DOCUMENT_DEACTIVATED = "KNOWLEDGE_DOCUMENT_DEACTIVATED"

    COMPLIANCE_CASE_CREATED = "COMPLIANCE_CASE_CREATED"
    COMPLIANCE_CASE_UPDATED = "COMPLIANCE_CASE_UPDATED"
    COMPLIANCE_CASE_ASSIGNED = "COMPLIANCE_CASE_ASSIGNED"
    COMPLIANCE_CASE_STATUS_CHANGED = "COMPLIANCE_CASE_STATUS_CHANGED"

    CUSTOMER_RISK_PROFILE_CREATED = "CUSTOMER_RISK_PROFILE_CREATED"
    CUSTOMER_RISK_PROFILE_UPDATED = "CUSTOMER_RISK_PROFILE_UPDATED"

    RISK_SCORING_RULE_CREATED = "RISK_SCORING_RULE_CREATED"
    RISK_SCORING_RULE_UPDATED = "RISK_SCORING_RULE_UPDATED"
    RISK_SCORING_RULE_STATUS_CHANGED = "RISK_SCORING_RULE_STATUS_CHANGED"

    CUSTOMER_RISK_SCORE_CALCULATED = "CUSTOMER_RISK_SCORE_CALCULATED"


class UserStatus(str, Enum):
    ACTIVE = "active"
    INACTIVE = "inactive"
    LOCKED = "locked"


class UserRole(str, Enum):
    ADMINISTRATOR = "Administrator"
    REVIEWER = "Reviewer"
    COMPLIANCE_OFFICER = "Compliance Officer"
    AUDITOR = "Auditor"


class AddressType(str, Enum):
    RESIDENTIAL = "residential"
    MAILING = "mailing"


class PreferredContactMethod(str, Enum):
    PHONE = "phone"
    EMAIL = "email"


class VerificationType(str, Enum):
    IDENTITY = "IDENTITY"
    ADDRESS = "ADDRESS"
    BUSINESS = "BUSINESS"


class VerificationStatus(str, Enum):
    NOT_STARTED = "NOT_STARTED"
    PENDING = "PENDING"
    UNDER_REVIEW = "UNDER_REVIEW"
    APPROVED = "APPROVED"
    REJECTED = "REJECTED"
    EXPIRED = "EXPIRED"


class DocumentCategory(str, Enum):
    IDENTITY = "IDENTITY"
    ADDRESS = "ADDRESS"


class DocumentStatus(str, Enum):
    UPLOADED = "UPLOADED"
    PROCESSING = "PROCESSING"
    VERIFIED = "VERIFIED"
    REJECTED = "REJECTED"
    EXPIRED = "EXPIRED"


class ReviewDecision(str, Enum):
    APPROVE = "APPROVE"
    REJECT = "REJECT"
    REQUEST_MORE_INFORMATION = "REQUEST_MORE_INFORMATION"


class OCRProcessingStatus(str, Enum):
    SUBMITTED = "submitted"
    PROCESSING = "processing"
    COMPLETED = "completed"
    FAILED = "failed"


class ExtractionStatus(str, Enum):
    SUBMITTED = "submitted"
    PROCESSING = "processing"
    COMPLETED = "completed"
    FAILED = "failed"


class ExtractionReviewStatus(str, Enum):
    PENDING_REVIEW = "PENDING_REVIEW"
    APPROVED = "APPROVED"
    REJECTED = "REJECTED"


class AIPromptStatus(str, Enum):
    ACTIVE = "ACTIVE"
    INACTIVE = "INACTIVE"


class AIFunction(str, Enum):
    DOCUMENT_EXTRACTION = "DOCUMENT_EXTRACTION"
    COMPLIANCE_REVIEW = "COMPLIANCE_REVIEW"
    RISK_ANALYSIS = "RISK_ANALYSIS"
    CUSTOMER_SUPPORT = "CUSTOMER_SUPPORT"

    CASE_SUMMARY = "CASE_SUMMARY"
    CUSTOMER_SUMMARY = "CUSTOMER_SUMMARY"
    DOCUMENT_REVIEW_SUMMARY = "DOCUMENT_REVIEW_SUMMARY"
    COMPLIANCE_NOTES = "COMPLIANCE_NOTES"


class AIInteractionStatus(str, Enum):
    PENDING = "PENDING"
    COMPLETED = "COMPLETED"
    FAILED = "FAILED"


class AIResourceType(str, Enum):
    CUSTOMER = "CUSTOMER"
    VERIFICATION_CASE = "VERIFICATION_CASE"
    DOCUMENT = "DOCUMENT"


class KnowledgeDocumentCategory(str, Enum):
    COMPLIANCE_POLICY = "COMPLIANCE_POLICY"
    INTERNAL_PROCEDURE = "INTERNAL_PROCEDURE"
    PRODUCT_DOCUMENTATION = "PRODUCT_DOCUMENTATION"
    REGULATORY_GUIDELINE = "REGULATORY_GUIDELINE"


class KnowledgeDocumentStatus(str, Enum):
    ACTIVE = "ACTIVE"
    INACTIVE = "INACTIVE"


class ComplianceCaseType(str, Enum):
    CUSTOMER_REVIEW = "CUSTOMER_REVIEW"
    AML_ALERT = "AML_ALERT"
    VERIFICATION_ISSUE = "VERIFICATION_ISSUE"
    SUSPICIOUS_ACTIVITY_REVIEW = "SUSPICIOUS_ACTIVITY_REVIEW"
    MANUAL_INVESTIGATION = "MANUAL_INVESTIGATION"


class ComplianceCasePriority(str, Enum):
    LOW = "LOW"
    MEDIUM = "MEDIUM"
    HIGH = "HIGH"
    CRITICAL = "CRITICAL"


class ComplianceCaseStatus(str, Enum):
    OPEN = "OPEN"
    ASSIGNED = "ASSIGNED"
    UNDER_REVIEW = "UNDER_REVIEW"
    ESCALATED = "ESCALATED"
    RESOLVED = "RESOLVED"
    CLOSED = "CLOSED"


class CustomerRiskLevel(str, Enum):
    LOW = "LOW"
    MEDIUM = "MEDIUM"
    HIGH = "HIGH"
    CRITICAL = "CRITICAL"


class RiskRuleOperator(str, Enum):
    EQUALS = "EQUALS"
    NOT_EQUALS = "NOT_EQUALS"
    IN = "IN"
    NOT_IN = "NOT_IN"
    GREATER_THAN = "GREATER_THAN"
    GREATER_THAN_OR_EQUAL = "GREATER_THAN_OR_EQUAL"
    LESS_THAN = "LESS_THAN"
    LESS_THAN_OR_EQUAL = "LESS_THAN_OR_EQUAL"
    EXISTS = "EXISTS"
    NOT_EXISTS = "NOT_EXISTS"
